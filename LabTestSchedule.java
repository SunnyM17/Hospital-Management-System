	...
	public LabTestSchedule(JFrame frame, Assistant assistantUser, Patient patientUser, String name) {
		...
		doneBtn.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				FileSystem tempFile = new FileSystem(Integer.toString(patientUser.getUserID()));
				...
				if(checkSelection == 1) {
					for(int i=0; i< time.size(); i++) {
						if(time.get(i).isSelected() && i>=0 && i<=8) {
							tempFile.writeToFile(file, "Appointment: " + "Monday " +time.get(i).getLabel() + " " + name);
						}
						else if(time.get(i).isSelected() && i>=9 && i<=17) {
							tempFile.writeToFile(file, "Appointment: " + "Tuesday "+ time.get(i).getLabel() + " " + name);
						}
						else if(time.get(i).isSelected() && i>=17 && i<=26) {
							tempFile.writeToFile(file, "Appointment: " + "Wednesday "+ time.get(i).getLabel() + " " + name);
						}
						else if(time.get(i).isSelected() && i>=26 && i<=35) {
							tempFile.writeToFile(file, "Appointment: " + "Thursday "+ time.get(i).getLabel() + " " + name);
						}
						else if(time.get(i).isSelected() && i>=35 && i<=44) {
							tempFile.writeToFile(file, "Appointment: " + "Friday "+ time.get(i).getLabel() + " " + name);
						}
					}
				}
		...
		JLabel lblNewLabel_6 = new JLabel("Select only one timing!");
